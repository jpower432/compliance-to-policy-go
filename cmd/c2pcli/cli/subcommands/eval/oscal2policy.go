...
func NewOSCAL2Policy(logger hclog.Logger) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runOSCAL2Policy(cmd.Context(), option)
		},
	}
	options.BindPluginFlags(command.Flags())
	options.BindOSCALFlags(command.Flags())
	return command
}

func runOSCAL2Policy(ctx context.Context, option *options.Options) error {
