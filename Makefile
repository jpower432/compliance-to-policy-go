...
# github.com/compliance-to-policy-bundle:$VERSION and github.com/compliance-to-policy-catalog:$VERSION.
IMAGE_TAG_BASE ?= github.com/compliance-to-policy
...
.PHONY: build-plugins
build-plugins:
	GOOS=$(GOOS) GOARCH=$(GOARCH) go build -o ./bin/c2pcli_$(VERSIONED_SUFFIX) ./cmd/kyverno-plugin
	GOOS=$(GOOS) GOARCH=$(GOARCH) go build -o ./bin/c2pcli_$(VERSIONED_SUFFIX) ./cmd/ocm-plugin
...
test-controllers: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test ./controllers/... -coverprofile cover.controllers.out -p=1

##@ Build

.PHONY: build-controllers
build-controllers: generate fmt vet ## Build manager binary.
...
.PHONY: docker-build
docker-build: test-controllers ## Build docker image with the manager.
	docker build -t ${IMG} .
...
.PHONY: docker-buildx
docker-buildx: test-controllers ## Build and push docker image for the manager for cross-platform support
...
## For development in a non OCM Hub
.PHONY: install-ocm-related-crds
install-ocm-related-crds:
	kubectl apply -f config/ocm

.PHONY: uninstall-ocm-related-crds
uninstall-ocm-related-crds:
	kubectl delete --ignore-not-found=$(ignore-not-found) -f config/ocm
...
.PHONY: get-policy-resources
get-policy-resources:
	kubectl get policy,placementrule,placementbinding

.PHONY: delete-policy-resources
delete-all-policy-resources:
	kubectl delete policy,placementrule,placementbinding --all
